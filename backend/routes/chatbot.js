...
router.post("/embed-movies", async (req, res) => {
  try {
    ...
    res.json({
      message: "Movies embedded successfully",
...
`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You recommend movies based only on the search results provided.",
        },
    ...
    // guard against the LLM hallucinating movies outside the provided context
    const validIds = new Set(movies_json.map((m) => m.metadata.id));
    recommendations.recommendations = (
      recommendations.recommendations || []
    ).filter((r) => validIds.has(r.movie?.id));
